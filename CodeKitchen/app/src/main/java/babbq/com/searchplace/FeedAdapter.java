...
import babbq.com.searchplace.data.DataLoadingSubject;
import babbq.com.searchplace.data.CodeKitchenItem;
import babbq.com.searchplace.data.PlaidItemComparator;
import babbq.com.searchplace.data.model.Post;
...
//    private final boolean pocketIsInstalled;
    ...
    private List<CodeKitchenItem> items;
...
//                        layoutInflater.inflate(R.layout.dribbble_shot_item, parent, false));
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder holder, int position) {
        if (position < getDataItemCount()
                && getDataItemCount() > 0) {
            CodeKitchenItem item = getItem(position);
...
//            } else
    ...
    private CodeKitchenItem getItem(int position) {
    ...
    private void add(CodeKitchenItem item) {
        items.add(item);
    }
    ...
    public void addAndResort(Collection<? extends CodeKitchenItem> newItems) {
        // de-dupe results as the same item can be returned by multiple feeds
        boolean add = true;
        for (CodeKitchenItem newItem : newItems) {
            int count = getDataItemCount();
            for (int i = 0; i < count; i++) {
                CodeKitchenItem existingItem = getItem(i);
    ...
    private void expandPopularItems() {
        ...
        for (int i = 0; i < count; i++) {
            CodeKitchenItem item = getItem(i);
...
//            } else
    ...
    public void removeDataSource(String dataSource) {
        int i = items.size() - 1;
        while (i >= 0) {
            CodeKitchenItem item = items.get(i);
